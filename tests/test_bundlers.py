...
def test_write_about_json_without_conda_on_path(testing_workdir, testing_metadata):
    with put_bad_conda_on_path(testing_workdir):
        ...
        bundlers._write_about_json(testing_metadata)
...
def test_get_short_path(testing_metadata):
    ...
    assert bundlers._get_short_path(testing_metadata, "test/file") == "test/file"

    # Test for noarch: python
    meta = get_noarch_python_meta(testing_metadata)
    assert bundlers._get_short_path(meta, "lib/site-packages/test") == "site-packages/test"
    assert bundlers._get_short_path(meta, "bin/test") == "python-scripts/test"
    assert bundlers._get_short_path(meta, "Scripts/test") == "python-scripts/test"


def test_is_no_link():
    no_link = ["path/1", "path/2"]
    assert bundlers._is_no_link(no_link, "path/1") is True
    assert bundlers._is_no_link(no_link, "path/nope") is None


def test_has_prefix():
    files_with_prefix = [("prefix/path", "text", "short/path/1"),
                         ("prefix/path", "text", "short/path/2")]
    assert bundlers._has_prefix("short/path/1", files_with_prefix) == ("prefix/path", "text")
    assert bundlers._has_prefix("short/path/nope", files_with_prefix) == (None, None)
...
def test_sorted_inode_first_path(testing_workdir):
    ...
    assert bundlers._get_inode_paths(files, "one", testing_workdir) == ["one", "one_hl"]
    assert bundlers._get_inode_paths(files, "one_hl", testing_workdir) == ["one", "one_hl"]
    assert bundlers._get_inode_paths(files, "two", testing_workdir) == ["two"]
...
def test_create_info_files_json(testing_workdir, testing_metadata):
    ...
    bundlers._create_info_files_json_v1(testing_metadata, info_dir, testing_workdir, files,
                                    files_with_prefix)
...
def test_sanitize_channel():
    ...
    assert bundlers._sanitize_channel(test_url) == 'https://conda.anaconda.org/t/<TOKEN>/somechannel'
...
def test_find_prefix_files(testing_workdir):
    ...
    assert len(list(bundlers._have_prefix_files(files, testing_workdir))) == len(files)
