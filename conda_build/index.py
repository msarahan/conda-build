...
def get_converse_format_path(fn, subdir_path, convert_if_not_present=True):
    ...
    if not os.path.lexists(os.path.join(subdir_path, converted_fn)):
        if convert_if_not_present:
            conda_package_handling.api.transmute(fn, fn.replace(converted_fn), out_folder=subdir_path)
        return None
    return converted_fn
...
class ChannelIndex(object):
    ...
    def _extract_to_cache(self, subdir, fn):
        ...
        subdir_path = join(self.channel_root, subdir)
        if fn.endswith(".conda"):
            tar_path = join(subdir_path, get_converse_format_path(fn, subdir_path, convert_if_not_present=True))
            new_format_path = fn
        else:
            tar_path = join(subdir_path, fn)
            new_format_path = get_converse_format_path(tar_path, subdir_path, convert_if_not_present=False)
    ...
    def _load_index_from_cache(self, subdir, fn, stat_cache):
        ...
        new_format_path = get_converse_format_path(fn, subdir_path, convert_if_not_present=False)
